...
def build_relation_extract_prompt(*, work_title: str, characters: list[dict], limit: int) -> str:
    return dedent(
        ...
        {{
          ...
          "characters": [
            {{
              ...
              "name": "캐릭터명",
              "role": "관계도 카드에 표시할 역할",
              "profile_label": "관계도 카드에 표시할 직업/소속/정체성",
        ...
        - role은 주인공/조력자/악역 같은 작품 내 역할을 짧게 쓰고, 직업/소속/정체성은 profile_label에 따로 쓴다.
        - profile_label은 입력에 있는 직업/소속/정체성을 우선 사용한다. 없으면 세부 설정에서 명확히 확인되는 경우만 보완한다.
